"""precis-bio plugin migrations.

Discovered via the ``precis.migrations`` entry-point group (namespace
``precis_bio``). The core migration runner (:class:`precis.store.Migrator`)
...
``kinds`` reference table + the 0023 plugin column) is in place first.
"""
