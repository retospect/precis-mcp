...
def provenance_state(text: str) -> str:
    ...
    or computational evidence — :data:`precis.utils.mentions.
    COMPUTED_EVIDENCE_KINDS`), ``"pending"`` (cites a ``[fi<id>]`` finding —
    a source still being chased), or ``"unsourced"`` (no citation at all).
    Keys off each chip's ``kind`` from :func:`_ref_chips` rather than
    re-parsing the citation grammar (DRY — same parser the "Cited sources"
    rail keys off). Unlike that rail, a chunk-form cite (``[pc10]``,
    ``is_chunk=True``) still counts as ``"sourced"`` — a chunk citation is
    real grounding evidence."""
    chips = _ref_chips(text)
    kinds = {c.kind for c in chips}
    if kinds & ({"paper", "patent"} | mentions.COMPUTED_EVIDENCE_KINDS):
        return "sourced"
    if "finding" in kinds:
        return "pending"
    return "unsourced"
...
@router.post("/drafts/{ident}/remarkable-papers")
async def send_remarkable_papers_route(request: Request, ident: str) -> Response:
    """Start a ``remarkable_papers_send`` job — send every cited source PDF
    ...
    return await redirect_or_error(
        ...
        {
            "kind": "job",
            "job_type": "remarkable_papers_send",
            "parent_id": _job_parent(store, ref),
            "params": params,
            # Login-scoped: two users sending the same draft target
            # different tablets, so they must not coalesce into one job.
            "idem_key": f"remarkable_papers_send:{slug}:{login or 'shared'}",
        },
        redirect=f"/drafts/{ident}",
        error_title="reMarkable send error",
    )
...
@router.post("/drafts/{ident}/remarkable-reading")
async def send_remarkable_reading_route(request: Request, ident: str) -> Response:
    """Start a ``remarkable_reading_send`` job — typeset each cited source
    ...
    return await redirect_or_error(
        ...
        {
            "kind": "job",
            "job_type": "remarkable_reading_send",
            "parent_id": _job_parent(store, ref),
            "params": params,
            # Login-scoped: two users sending the same draft target
            # different tablets, so they must not coalesce into one job.
            "idem_key": f"remarkable_reading_send:{slug}:{login or 'shared'}",
        },
        redirect=f"/drafts/{ident}",
        error_title="reMarkable send error",
    )
...
async def merge_prev_block(
    ...
    # One atomic store call (gr176088 part 2b) — append onto prev + retire
    # this block happen inside a single transaction, so a conflicting
    # concurrent edit to prev can never land between them and get lost.
    # base_sha = the sha of the prev text this handler read above; a
    # concurrent edit of prev raises BadInput instead of silently
    # clobbering it — mirrored 409 shape, same as split_block above.
    try:
        store.drafts.merge_prev_block(
            handle, prev.handle, text, base_sha=content_sha(prev.text or "")
        )
    except BadInput as exc:
        if "changed since you read" in str(exc):
            return JSONResponse({"ok": False, "error": str(exc)}, status_code=409)
