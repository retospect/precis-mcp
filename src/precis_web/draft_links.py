...
def chunk_links(
    ...
    *,
    ...
    ``links_out``/``links_in`` split ``store.drafts.chunk_connections``'s rows by
    ...
    ``store.drafts.anchored_todos`` — the change-request todos anchored directly at
    ...
    ``store.drafts.chunk_connections``/``store.drafts.anchored_todos`` already returned
