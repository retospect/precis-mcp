...
| range with a typical value | typical value first, then the full range, joined by `across`: `≈9 GPa across 9–12 GPa`. Never state a bare midpoint on its own — a computed mean is an arithmetic artifact, not a measurement (authoring judgment for *when* to use this shape: [[precis-taproot-mint-help]]'s numeric-value policy). |
| unit placement in a range | one unit, stated once, after the second endpoint: `9–12 GPa`. Never repeat it (`9 GPa–12 GPa`) or attach it to the first endpoint only (`9GPa–12 GPa`). |
