...
| `se` | `unicycle-printed-v1` | Structural/mechanical design in metres — block tree with envelopes, ports, joints (incl. prestressed `axial` members), measures, BOM; views check what you claimed (validate/drc/stability/fasten/freedom). See `precis-se-help` (surface) + `precis-se-design-help` (workflow). | store |
