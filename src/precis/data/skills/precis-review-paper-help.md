...
Run [[precis-adversarial-reviewer]]'s checklist as the floor —
unsupported claims, missing controls, overgeneralisation,
statistics, selective citation, reproducibility holes, internal
inconsistency. This pass adds:

1. **Verifier-loop check.** Every cite token — `[pc<id>]` paper
...
2. **Counterargument audit.** For each major claim, ask "what's the
...
3. **Methodology against best practice.** Each method gets
...
4. **Quantification gaps.** Claims missing numbers, numbers
   missing error bars, error bars missing N, ranges missing the
   distinguishing conditions — each one numbered.

5. **Scope drift.** Did the draft promise X in the abstract and
   deliver Y in the conclusion? Did a section's title not match
   its content? Note explicitly.

6. **Self-consistency.** Does Table 2 agree with Figure 3? Does the
   discussion's "we found X" align with the results' "we measured
   X ± Δ"? Numbered point per discrepancy.

7. **Label severity honestly.** Each finding is labelled:
   - **BLOCKER** — claim doesn't hold, missing analysis,
     significant methodological flaw. Fix before publication.
   - **MEDIUM** — citation drift, missing context, weak
     argument. Addressable but not fatal.
   - **LOW** — wording, formatting, minor reference style.
     Not a real issue.
   Low-severity findings stay under 10% of total; if your review is
   90% low-severity, you didn't read for substance.

8. **No sycophancy openers.** Skip "The authors have done excellent
...
```
1. **BLOCKER** [cite:142 doesn't support the claim]
...
2. **MEDIUM** [missing counterargument]
...
3. **BLOCKER** [unsupported quantification]
...
```
## Summary
- Blocker findings: N (fix before resubmit)
- Medium findings: N
- Low findings: N (omit if not asked)
...
- 90% low-severity findings. The point is substance.
