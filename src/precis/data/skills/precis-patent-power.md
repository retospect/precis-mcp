...
**The local leg doesn't parse CQL — it keyword/embedding-matches `q=`
as literal free text.** Field prefixes, booleans, and quoted numbers
all stay in the string, so a stray token like the `2024` in `pd
within "2020 2024"` pulls unrelated already-ingested patents that
happen to mention 2024. Only the remote (`[ops]`) leg actually parses
the CQL — read it as the real signal and don't be surprised by a thin
or off-topic `[local]` set. `reach='remote'` skips the noisy local leg
entirely on a query with digits or operators in it.
