...
| `add_adsorbate` | `species:"H"\|"O"\|"N"\|"OH"\|"H2O"\|"NH"\|"NH2"\|"NH3"`, `site:{…}` (as above), `height?`, `rotate?` (° about the surface normal) | place a whole adsorbate **group** from one named site: the binding atom lands where `add_atom_site` would put it, the rest follow at the species' gas-phase internal geometry, and the intra-group bonds are minted. The upright orientation is a **pre-relaxation starting guess**, not an adsorbed equilibrium — relax after placing. Use it for coverage series (θ_H, θ_O, θ_OH) and explicit-water spectators instead of hand-placing each H |
