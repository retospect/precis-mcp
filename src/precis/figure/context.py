"""The prepared-context assembler for diagram editing (ADR 0057).

Given a diagram's source chunk, produce the block the model edits *inside*:
every depicting element (its id + geometry) paired with the chunk it is bound
to, followed by the linked chunk bodies inlined. This is the diagram analogue
of the planner's ``_render_anchor_context`` — generalised from one anchor
chunk to a whole diagram's element set.

Kind-agnostic in spirit (it reads ``store.element_bindings`` + parses SVG for
coordinates); it lives under ``figure/`` for slice 2 and moves into the shared
``diagram/`` core when figure + mermaid are factored (slice 3). Coordinates
come from the SVG element geometry; a mermaid instance will supply topology
(node + edges) instead.
"""
...
from precis.figure.svg import elements
...
def render_diagram_context(store: _StoreLike, node_chunk_id: int, svg: str) -> str:
    ...
    lines = [
        ...
        "edges); keep every element's geometry faithful to the linked source.",
    ]
    for b in binds:
