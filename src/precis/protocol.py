...
class KindSpec:
    ...
    #: Organizational placement. ``"artifact"`` — an authored
    ...
    #: folders until deliberately promoted. NB ``placement`` does *not*
    #: alter cross-kind search fan-out, which stays keyed on
    #: ``supports_search_hits``.
    placement: Literal["artifact", "corpus", "stream", "system"] = "stream"
    ...
    #: ``docs/backlog/mcp-cold-start-token-budget.md`` Phase 5 step 2:
