...
def check_workspace_compiles(
    ...
    * latexmk isn't installed (we log + skip rather than block the
    ...
    # NB the Path-B refs.bib regeneration (utils/bib_gen, keyed off
    # kind='citation' refs + the \citequote authoring model) was retired:
    # a draft-bound project's bibliography is now generated by the draft
    # export engine (export/latex.build_bib) from the inline [pc<id>]
