...
A note is name-keyed (like everything else in this kind) and one of three
kinds: a **question** ("what bearing bore?"), an **answer**, or a
**decision**. ``re`` links an answer/decision to the note it responds to;
``about`` anchors a note to the design objects it concerns (``'block'`` or
``'block.measure'`` names — name-keyed text, resolved only at read time:
a dangling anchor is the interview view's honest annotation, never a
write-time error, the relation-source posture).
...
ledger append-shaped (corrections are new notes; ``remove_note`` exists
for genuine retractions) and makes ``view='interview'`` a truthful
timeline — ``created_at`` is carried across saves by
:mod:`precis_se.persist` precisely so this ordering survives the
retire-all/reinsert-all save model.
"""
...
class NoteSpec:
    ...
    ``about`` is a list of ``'block'`` / ``'block.measure'`` anchor names.
...
def validate_about(raw: Any) -> list[str]:
    ...
    **existence** is deliberately not checked (module docstring)."""
    ...
    if not isinstance(items, list):
        raise NoteError(
            f"'about' must be a list of 'block' / 'block.measure' anchor "
            f"names, got {raw!r}"
        )
    out: list[str] = []
    for item in items:
        if not isinstance(item, str) or not item.strip():
            raise NoteError(
                f"'about' anchors must be non-empty strings "
                f"('block' or 'block.measure'), got {item!r}"
            )
...
def dangling_anchors(notes: list[NoteSpec], resolve: Any) -> list[tuple[str, str]]:
    ...
    knows the tree). Read-time honesty, not an error."""
