---
description: Summarize the next logical steps and emit a copy-paste recovery prompt to resume after /compact. Use before compacting, or any time you want a clean-context restart point.
...
restart**: the next logical steps, plus a self-contained recovery prompt they
copy, run `/compact`, then paste as their first message to resume with full
orientation intact. Compaction summarizes away free-text working context — the
recovery prompt is what carries it across, so it must stand on its own and lean
on **durable artifacts**, not this conversation's transcript.
...
4. **Close with the nudge.** After the block, one line:
   ```
   Copy the block above → /compact → paste it as your first message.
   ```
