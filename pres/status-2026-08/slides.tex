...
\begin{frame}{Taproot: claims with receipts}
  ...
  \begin{itemize}
    ...
    \item[\sidea] Evidence must come from the \emph{meat} of the paper ---
          ``X was doped with Y and we measured Z'', not ``it is known
          that\ldots''. The section categorizer will strike prior-work and
          review sections from the pool of admissible sources
    \item[\sidea] Found the hard way: a claim whose support traced back to a
...
\begin{frame}{The argument graph}
  ...
  \begin{itemize}
    ...
    \item[\srough] Status, honestly: machinery shipped end-to-end,
          producers still default-off in prod --- ramping deliberately
  \end{itemize}
\end{frame}
