...
- **DEFERRED — local-first revisit** (Reto: "in a bit"): failover-at-load is BUILT (router.py:1281-1285: a saturated/`paused` local slot retries the hosted OSS endpoint before the next rung); resolve_chain supports a LOCAL rung `{"placement":"local","model":"<served_by id>","transport":...}` (router.py:1078-1160, `placement:"local"` authoritative). To localize a tier later: stand up the GGUF on a host's llama-swap (auto-advertises served_by), prepend a `placement:"local"` rung before the cloud rung — cloud is then the high-load failover. Host notes: balthazar is the SMALL Mac (34GB, ~3GB free, runs a 35B — repurpose that slot, don't co-locate); melchior 206GB but is the claude_inproc SPOF; spark GPU but busy/flaky ssh. BLOCKER for local GLM: GLM-4.7-Flash no-think key for llama.cpp is UNCONFIRMED (Unsloth doc silent — the hosted OpenRouter `reasoning.enabled:false` works, but the local chat-template switch does not; `_dispatch_local` NOTE flags it) — verify before trusting local classify. GGUF: `unsloth/GLM-4.7-Flash-GGUF` UD-Q4_K_XL ~10GB.
- **Phase C tier sweep — DONE in code (this worktree, pending ship+deploy).** Deleted the 5 legacy location-coupled Tier members (LOCAL_SMALL/LOCAL_BIG/CLOUD_SMALL/CLOUD_MID/CLOUD_SUPER); everything routes on the 4 capability tiers (FRONTIER/BIG/MEDIUM/SMALL). plan_tick + quest fallbacks repointed to `Tier.BIG` (the `local` LLM alias now → BIG); router grew `tier_from_str()` + `_LEGACY_TIER_ALIASES` so a *stored* pre-ship legacy tier string (in-flight quest `meta.loop.tier` / baked job `meta.params.tier`) degrades onto its analogue instead of raising. Fixed 2 latent bugs (quest/loop.py + quest_tick `_phase_tick` had `"local-big"` string defaults → now `"big"`). Full suite green (9270). **GOVERNANCE NOTE:** `LLM:local`-tagged ticks now route to OpenRouter (glm-5.2) instead of the on-prem 80B — accepted under "make it remote, the lot of it", revisit at the local-first pass. **POST-DEPLOY:** drop the now-dead `llm.chain.local-big` app_settings row (nothing constructs Tier.LOCAL_BIG anymore; `llm.chain.big` governs).
- **Latent bugs to fix WHEN local-first is reinstated** (both masked today because all rungs are paid/remote; surfaced by the Phase-C review, Opus-verified): (1) `policy.py::_model_price` free-price fast path is `tier_floor.startswith("local")` — DEAD since migration 0090 relabeled every `llm` card's `tier_floor` to frontier/big/medium/small (nothing starts with "local"). A genuinely-free local served card now returns `inf` → dropped from the Pareto set (`_next_better`) + ranked LAST in `select_offering`'s cost sort, so a paid cloud card beats a free local one. Fix: classify "free" by the offering's transport (litellm / served_by slot) or absent price, not the `tier_floor` string. Pre-existing (0090), NOT introduced by the Phase-C ship. (2) `bands.py` collapsed `LOCAL_BIG`(FREE) + `CLOUD_MID`(CHEAP) into one `BIG: Band(CHEAP)` row, and `breaker.gate_tier` gates on `is_paid(tier)` not the resolved transport — so once the $ cap trips, EVERY `BIG` call is refused, including one resolving through a free local `llm.chain.big` rung. Fix: gate on resolved transport, or split a free-local BIG classification. Both bite only when a free local rung backs BIG/SMALL again.
