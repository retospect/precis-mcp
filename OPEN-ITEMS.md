...
**The local-serving half is FIXED + PROD-VERIFIED (shipped+deployed `1c0d63b6` 2026-08-07, capacity-valve item 2).** `dispatch`/`dispatch_async` now acquire the slot under the chain rung's served model (`serve_model = ladder[0].model or model`), not the pre-chain tier/source alias (`summarizer`) — so on melchior SMALL reserves the `llm:qwen3.5-9b-q4_k_m` slot, gets the `:11445` endpoint, and `_dispatch_local` serves it locally. Verified in prod steady state: chain SMALL sources (`bib_parse`, classify, axis) land `placement=local, model=qwen3.5-9b-q4_k_m` (23 local / 56 in a 5-min window at 22:42Z; the rest correctly cloud — only melchior serves the 9B). Regression: `tests/test_llm_router.py::test_dispatch_acquires_slot_under_chain_rung_model_not_tier_alias`. (Original defect: `acquire(model)` used the pre-chain name, so `acquire("summarizer")` missed the served slot → no endpoint → cloud failover; chain SMALL ran 100% on OpenRouter `z-ai/glm-4.7-flash` until this fix. **Verification gotcha, recorded so it isn't repeated:** the two post-deploy checks reading "still 100% cloud" were false negatives — one window was mostly *pre*-deploy, the other landed in the llama-swap reload window right after the deploy bounce. Wait past the reload window, use absolute-UTC `ts` cutoffs, and note melchior logs are IST/UTC+5:30.)
...
forever (a parent whose job failed, or that still has a live child todo). The
fix drained 59 of them within minutes of deploying — those had been resolvable
all along and simply never reached — leaving **110** genuinely stuck. Now that
sampling is fair, each leaf is looked at roughly every `110/50` ≈ 2
...
Surfaced while wiring the reveal audit. `vault.events` says **287,301** reveals
of `OPENROUTER_API_KEY` since 2026-07-25 — ~22k/day, ~15/minute — against a
`_CACHE_TTL_SECONDS = 60.0` module cache that should cap it near 1,440/day
*per process*. Either far more processes are resolving it than we think, or the
cache is being bypassed (a fresh `_cache` per short-lived subprocess would do
it, as would any path calling `invalidate()` on a hot loop). Worth knowing
which: at this rate the audit table is the fastest-growing thing in the DB
(hence `vault.gc_events`), and a per-call decrypt is real DB work.
Now diagnosable — the `host` / `pid` / `process` columns from 0111 say exactly
...
**The credential surface bounds this — but it is two machines, not one.** A
Claude call needs a token, so enumerating tokens enumerates callers. The catch
is enumerating the *right* thing: a first sweep for `.claude_oauth_token` found
only melchior and concluded "one machine", which was wrong. Claude Code's real
...
## 🔍 Paper-page product gaps (surfaced by Reto 2026-08-06): residual polish

All three gaps shipped+deployed (see `git log`): backlinks panel, hub unacquirable read-through + claim reflection, and abstract self-healing pass. Only residual polish remains below.

One remaining gap found while Reto browsed `/papers/carbon20?chunk=3` (ref 42553; **112 body chunks — fully acquired**, **no abstract stored**, no override) and `/claim/fi192860` (a `TAPROOT:claim` **hub**). Read-path/render — no schema change. (Gap 1 — hub unacquirable read-through + claim-page reflection — and gap 3 — backlinks panel — both shipped; see `git log`.)

- **(2-residual) Abstract pass — Crossref/S2 fallback** · Status: open · Severity: polish · Owner: `src/precis/workers/openalex_enrich.py`, `ingest/openalex_meta.py` · (2a card_abstract rebuild + 2b OpenAlex miss-marker both shipped — see `git log`.) Remaining: a **Crossref / Semantic-Scholar abstract fallback** for DOIs OpenAlex genuinely lacks (a stamped `meta.openalex.miss` DOI never gets an abstract from anywhere). New Lane C fetch through `safe_fetch`; treat its own miss the same way (stamp so it isn't re-tried).
- **(3-residual) Backlinks panel — text-scan coverage + deep page** · Status: open · Severity: polish · Owner: `src/precis_web/routes/papers.py` (`_backlinks`) · The inline "Referenced by" panel shipped (Meta tab, kind-agnostic over the materialized `links.dst_ref_id` reverse index). **Blocker found 2026-08-06 (why the naive text-scan was NOT shipped):** there is **no trigram GIN index on `chunks.text`** (`pg_trgm` extension is loaded, but no index), so a read-time inbound scan (`chunks.text ~ '[pa<ref_id>]' / '[pc<our-chunk>]'`) is a **full sequential scan of the whole chunks table on the paper hot path** — a perf regression, not polish. Two viable directions, pick one: **(i)** add a `USING gin (text gin_trgm_ops)` index on `chunks` (forward migration; sizeable index + write overhead — measure) then union the bounded scan; or **(ii, preferred)** materialize inline `[pa]/[pc]` cites into `links` at draft-save (autolinker completeness) + a one-time backfill — no per-page scan, and it also fixes the **undercount** facet (carbon20 shows 1 `draft:cites` edge where the review cites it repeatedly; a materialized-once edge stays count=1). Also still open: **deep `/papers/<id>/backlinks` page** — per-kind filter + the citing sentence via `src_chunk_id` (v1 dedupes a source to one row with an edge count).
...
- **Sibling defect — promote-and-freeze (fix shipped this session):** the *inverse* of the demotion above. `axis:taproot` also **promotes** live `STATUS:tracing` findings by stamping `TAPROOT:claim` + `TAPROOTCASCADE:1` on them. Because `claim_tracing_findings` (`workers/chase.py`) excluded *all* `TAPROOT:claim`, those findings then froze out of the chase lifecycle — never advanced, never canonical. Repro: the 6 Malthus-draft claims `fi183933/183942/183943/183944/183945/183946` (created 2026-08-01, `derived-from` chains intact, **zero** evidence edges, cited by draft `184217`). **Fix:** the chase claim exclusion now skips only *marker-less* `TAPROOT:claim` (the `mint_hub` signature — `mint_hub` writes the claim tag but never a `TAPROOTCASCADE` marker; the classifier writes both together), so classifier-labeled tracing findings stay claimable. Deploy un-freezes the 6 — they resume tracing on their `derived-from` frontier (advance if it has chunks, else `waiting`→14-day abandon). Edge case noted: an axis-labeled claim with an *empty* chain will now `dead_chain(empty_chain)` when chased — acceptable (these 6 all carry chains). Deeper options still open: (#2) stop the axis pass labeling active-lifecycle findings; (#3) wire classification → real `mint_hub`.
...
- **Enable hub-refine + chase-trigger in prod (Phase 2)** · Status: open ·
  ...
  The former embedder-warm blocker (gripes 185401/196964) is cleared: the
  worker resolves its embedder as **remote** (verified live on melchior
  2026-08-07 — plist + process args), and `CHASETRIG_VERSION` 2 lazily
  re-sweeps the chunks marked against the cold index.
