...
- **Phase C tier sweep — SHIPPED+DEPLOYED (main `9140f7a2`, 2026-07-26).** Deleted the 5 legacy location-coupled Tier members (LOCAL_SMALL/LOCAL_BIG/CLOUD_SMALL/CLOUD_MID/CLOUD_SUPER); everything routes on the 4 capability tiers (FRONTIER/BIG/MEDIUM/SMALL). plan_tick + quest fallbacks repointed to `Tier.BIG` (the `local` LLM alias now → BIG); router grew `tier_from_str()` + `_LEGACY_TIER_ALIASES` (unit-tested) so a *stored* pre-ship legacy tier string (in-flight quest `meta.loop.tier` / baked job `meta.params.tier`) degrades onto its analogue instead of raising. Fixed 2 latent bugs (quest/loop.py + quest_tick `_phase_tick` had `"local-big"` string defaults → now `"big"`). Dead `llm.chain.local-big` bridge row dropped post-deploy. **GOVERNANCE NOTE:** `LLM:local`-tagged ticks now route to OpenRouter (glm-5.2) instead of the on-prem 80B — accepted under "make it remote, the lot of it", revisit at the local-first pass.
