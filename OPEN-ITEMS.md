...
## 🔍 Paper-page product gaps (surfaced by Reto 2026-08-06): claim-side trust reflection · abstract backfill

Two remaining gaps found while Reto browsed `/papers/carbon20?chunk=3` (ref 42553; **112 body chunks — fully acquired**, **no abstract stored**, no override) and `/claim/fi192860` (a `TAPROOT:claim` **hub**). All read-path/render — no schema change. (Gap 3 — the "who links here" backlinks panel — shipped; see `git log`.)

- **(1) Unacquirable read-through is a no-op for hub claims + the claim page never reflects it** · Status: open · Severity: bug · Owner: `src/precis/taproot/trust.py` (`_source_paper_override`/`_hub_trust`), `src/precis_web/claim_render.py` · Test: a hub citing a paper with `meta.unacquirable_override` renders `Ⓐ/✍`, not `⚠`; a lifecycle finding already does. The paper Meta-tab form promises "claims citing it stop reading as unverified," but `_source_paper_override` (`trust.py:228`) only follows a **lifecycle** finding's `meta.chain[-1]` frontier — for a **hub** it returns `None` by design, so declaring a *supporter* paper unacquirable does **not** soften the hub's badge (e.g. fi192860). Fix: fold supporter-paper overrides into `_hub_trust` (walk evidence papers, apply the softest override); keep the *declaration control* on the paper (it's a paper property — declare once, ripple to all citing claims — do **not** move it per-claim). Then **surface** it on `/claim/fi…`: `claim_render.py` reads no unacquirable signal directly (only `claim_trust`'s final label), so add a per-claim reflection line ("grounded on <paper>, declared unacquirable Ⓐ/✍ by <who>") — the *reflection*, not a second control.
- **(2) Store the abstract unconditionally (independent of full-text acquisition)** · Status: open · Severity: feature · Owner: ingest/metadata enrich (`ingest/openalex_meta.py`, Crossref/S2 paths) · Test: a paper with no OA PDF still lands `meta.abstract`. carbon20 has 112 chunks yet **no abstract** — a straight backfill miss. The abstract is publisher metadata (Crossref/S2/OpenAlex), almost always available even when the PDF isn't; it's *doubly* load-bearing for genuinely unacquirable papers (kroto/Iijima) because in Ⓐ mode **the abstract is the grounding** — an Ⓐ badge with no abstract is hollow. Fetch/store abstracts regardless of acquisition; show the supporting abstract next to an Ⓐ badge on the claim page. (Overlaps the "Auto-Ⓐ abstract-grounded verification" follow-on above — that pass needs the abstract present.)
- **(3-residual) Backlinks panel — text-scan coverage + deep page** · Status: open · Severity: polish · Owner: `src/precis_web/routes/papers.py` (`_backlinks`) · The inline "Referenced by" panel shipped (Meta tab, kind-agnostic over the materialized `links.dst_ref_id` reverse index; drafts→smartdraft, findings→`/claim/fi…`, papers→`/papers/…`). Two follow-ons remain: **(a) coverage** — the panel shows only *materialized* edges; some inline draft `[pc]/[pa]/[fi]` prose cites are scanned from chunk text on demand (`_citations_view`) and may lack a `links` row until the draft-save autolinker materializes them (carbon20 shows just 1 `draft:cites` edge where the citing review likely cites it repeatedly). Union a text-scan, or materialize at draft-save, for full recall. **(b) deep `/papers/<id>/backlinks` page** — per-kind filter + the citing sentence via `src_chunk_id` (v1 dedupes a source to one row with an edge count; the deep page shows each citing passage).
