...
- **(1-residual) Per-supporter unacquirable mark on `/claim`** · Status: open · Severity: polish · Owner: `src/precis_web/claim_render.py` (`_edge_row` + `render_claim_evidence`). The hub read-through + the claim-level reflection line shipped (`_hub_supporter_override` softens a clean hub whose every grounding supporter is unacquirable; `/claim/<head>` shows a calm "grounded on an unacquirable source — Ⓐ/✍" line with the note). Follow-on: mark the *specific* supporter row(s) that are unacquirable (batch-fetch supporter paper meta → thread `unacq_by_paper` into `_edge_row`) so the page names *which* paper, not just the mode+note. Cheap — the supporter refs are already fetched in the bulk path (`paper_refs`).
- **(2-residual) Abstract pass follow-ons** · Status: open · Severity: polish · Owner: `src/precis/workers/openalex_enrich.py`, `ingest/openalex_meta.py` · The self-healing `openalex_enrich` pass shipped (Lane A promotes an already-fetched `meta.openalex.abstract` to top-level; Lane B drips DOI'd papers through free OpenAlex; `enrich_ref` promotes the reconstructed abstract; state-map §passes). Two follow-ons: **(a) rebuild the `card_abstract` search card** for a promoted/filled abstract so it's embedded + searchable (touches the embedding cascade — DELETE+INSERT the `card_abstract` chunk so `embed:bge-m3` re-runs, per the append-only card rule); **(b) stamp an OpenAlex *miss*** — a DOI OpenAlex has no record for writes no `meta.openalex` block, so Lane B re-tries it every pass (wasted fetch, and it can occupy the newest-first batch). Stamp a `meta.openalex={v, tried_at}` miss-marker (or a `no_openalex` guard) to exclude tried-and-absent DOIs. Also: Crossref/S2 abstract fallback for DOIs OpenAlex genuinely lacks.
