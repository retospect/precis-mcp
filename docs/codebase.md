...
- `precis.anki` — Anki integration — headless AnkiWeb sync for the `anki` cloze kind (slice 2).
...
- `precis.handlers` — Handlers — one per kind.
...
- `precis.quest` — Quest layer runtime — reweighting (slice 2) and beyond.
...
- `precis.sim` — ``precis.sim`` — slice 1 of the sim-harness (``docs/proposals/sim-harness.md``).
...
- `precis.utils.llm` — The LLM routing layer (ADR 0046).
...
- `precis.workers` — Derived-artifact workers (per ADR 0007).
