...
> the owning package's docstring too). If a line here reads like "the current status of X,"
...
- **`refs` — one row per thing, discriminated by `kind`.** `todo`, `paper`,
  ...
  reserved `parent` relation; `requested`→job) — not
...
- **`chunks` — the body text, append-only.** Body rows (`ord >= 0`) are
  ...
  **NULL at ingest** — worker-filled), `chunk_summaries`, and
...
```
  precis add <input>
   ...
   refs row + chunks (embedding IS NULL)
        │  derived queue — workers pick up work by SQL, no blocking jobs
        ▼
...
- `asa_slack` — asa-slack — Slack bridge to Asa, routed through the LLM router.
...
- `precis.export` — Document export engines (LaTeX → Tier-B). The draft editable-document model.
- `precis.figure` — The ``figure`` kind — an interactive SVG canvas you draw *with* the model.
- `precis.fixer` — The laptop fixer loop.
...
- `precis.pcb` — The PCB *eyes* — pure-Python analysis over the netlist + placement
...
- `precis.structure.importers` — The adapter registry — external DFT library import's ETL seam for external DFT catalyst DBs.
- `precis.structure` — The ``structure`` kind — a legible atomistic cell + bond-graph IR.
...
- `precis.utils.llm` — The LLM routing layer — tiers, chains, catalog.
- `precis.utils.prompt` — Prompt assembler + module library.
...
- `precis_bio` — precis-bio — the protein / structure-prediction tool-pack.
- `precis_chem.migrations` — precis-chem plugin migrations.
- `precis_chem` — precis-chem — the chemistry / protein tool-pack.
...
- Coined/overloaded terms → `docs/glossary.md`
