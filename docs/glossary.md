...
- **persona** — the voice/stance prompt loaded per review lens or producer (review, oracle, narration). → `src/precis/workers/planner_prompt.py::_load_review_persona`
...
- **lane** — job parent lane (intent vs compute) · morning-brief lane (news/recall/quest) · LLM-router execution lane (route_log groups by lane/pass/ref) · repo-dev/gripe-intake lane (the fixer). → `src/precis/handlers/job.py` · `src/precis/reading/briefing_cast.py` · `src/precis/route_log.py` · `src/precis/fixer/intake.py`
- **dispatch** — the seven-verb `Hub` registration/dispatch table (`src/precis/dispatch.py`) · the `dispatch` worker (mints jobs from doable todos) · `runtime.dispatch` (in-process MCP verb call) · `dispatch(LlmRequest)` (the LLM router). → `src/precis/dispatch.py` · `src/precis/workers/dispatch.py` · `src/precis/runtime/dispatch.py` · `src/precis/utils/llm/router.py`
...
- **lens** — review lens (`flow`/`cites`/`structure`) · oracle lens (wisdom-tradition sampling policy) · citation-graph recall lens · auto-lens (an inferred **eye**). → `src/precis/quest/review_fanout.py` · `src/precis/utils/oracle_lens.py` · `src/precis/backfill/citation_lens.py` · `src/precis/workers/working_set.py`
