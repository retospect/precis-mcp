...
- **withheld edge** — an inbound evidence edge neither verified-by-refine (`links.meta['support']`) nor human-signed-off (`links.meta['publish_signoff']`); each one blocks the registry POST — no mute button. → `src/precis/nanopub/preflight.py`
...
- **trust tier** — the epistemic axis on `search(kind='finding', trust=…)` — `signed` / `verified` / `disputed` — orthogonal to `status=`, which is the chase lifecycle and says nothing about whether anyone checked the claim. → `src/precis/handlers/finding.py`
