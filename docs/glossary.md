...
- **emits_card / "a card is a vector"** — a per-handler class flag (`emits_card = True`): the kind emits a `card_combined` chunk (ord=-1) so the ref itself embeds + searches. → `src/precis/handlers/quest.py` · `src/precis/store/_blocks_ops.py::upsert_card_combined`
...
- **chunk** — the atomic content row (`chunks` table): body rows `ord >= 0`, card variants `ord < 0`. (legacy: *block* — the store facade still spells it `store.blocks`/`BlockStore` until the facade rename lands; there is no `blocks` table.) → `src/precis/store/_blocks_ops.py` · `docs/codebase.md`
- **task** — legacy web-surface synonym for a `todo` (the Tasks tab renders the todo tree; full rename scheduled, `docs/backlog/vocab-compaction-stages.md` Stage E). The machine execution unit is a `job`, never "task". → `src/precis_web/routes/tasks.py`
- **retire** — take a row out of service, kept for recovery (`retired_at` on chunks/cad/pcb tables). (legacy: refs still spell it `deleted_at`/`soft_delete_ref` until the Stage E migration — same operation.) → `src/precis/store/_refs_ops.py`
...
- **tier** — classifier tiers (0/1/2) · review tiers (nursery/structural/deep_review) · search tiers (Tier 1 RRF / Tier 2 good-search) · LLM tiers (`tier_floor`, `gate_tier`, opus/sonnet/haiku) · quest fidelity tiers (see **tier ladder**) · registry maturity tiers (component/material). → `src/precis/workers/classify.py` · `src/precis/workers/review.py` · `src/precis/utils/llm/router.py`
- **card** — an embedding chunk (`card_combined` / `card_glossary`, the searchable vector) · an Anki flashcard (`card_forge` mints these) · a catalog entry (`llm` / `quest` / `concept`). → `src/precis/store/_blocks_ops.py::upsert_card_combined` · `src/precis/handlers/anki.py` · `src/precis/handlers/llm.py`
...
- **dispatch** — the seven-verb `Hub` registration/dispatch table (`src/precis/dispatch.py`) · the `dispatch` worker (mints jobs from doable todos) · `runtime.dispatch` (in-process MCP verb call). The LLM router's entry point is `route(LlmRequest)`, no longer a dispatch. → `src/precis/dispatch.py` · `src/precis/workers/dispatch.py` · `src/precis/runtime/dispatch.py` · `src/precis/utils/llm/router.py::route`
...
- **watch** — a `level:recurring` todo whose `meta.schedule` drives the per-minute spawner (see **watch** in Coined terms) · unrelated `precis watch` CLI (`cli/watch.py`): a directory watcher that auto-ingests dropped PDFs (`watchdog`-based, no todo/schedule involvement). → `src/precis/workers/schedule/worker.py` · `src/precis/cli/watch.py`
