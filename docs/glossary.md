...
- **workbench turn** — one tool-less chat exchange on a design page: digest + clicked handles in, `{ops, rationale}` out, vetted against the kind's op roster; pure se ops auto-apply as one revision, store-aware se ops and every structure op become a proposal for the human Apply. Transcript = one `conv` per design, `turn` handle `design-chat-<slug>~N`. → `src/precis_web/design_turn.py` · `src/precis_web/design_chat.py`
