...
- **unit enclave** — a package granted a non-SI internal unit convention, ONLY while every identifier self-names its unit and every cross-package API converts to SI (the `fit_classes.hole_m` pattern). Granted 2026-09-12: `pcb` (mm — gerber/excellon/IPC/JLC are mm at both pipe ends) and `structure` + the atomistic layers of its consumers (Å/eV — ASE-woven compute core, CIF/DFT serializers). Everything else: bare numbers are SI. → `src/precis/utils/units.py` module docstring
...
- **disputes** — the free, non-blocking "these two claims appear to conflict" edge (hub↔hub or paper→hub; migration 0151), fileable by anyone, never a demerit, rendered as an open question. Distinct from `contradicts`, which post-split is adjudication-derived only (Part 2, not built) and blocks a hub's nanopub mint. → `src/precis/taproot/hub.py::link_claims`/`reattach_as_disputes` · `src/precis/migrations/0151_disputes_relation.sql`
...
- **revision** (design) — one save of an `se`/`structure` design as a `design_revisions` row: the ops that produced it, the `turn` that asked for it, and (se) a `rev-N` checkpoint snapshot; `?rev=N` on the design page is the scrubber, past revisions are read-only. Distinct from **envelope revision** above. → `src/precis/design/history.py`
- **workbench turn** — one tool-less chat exchange on a design page: digest + clicked handles in, `{ops, rationale}` out, vetted against the kind's op roster (a validator rejection is fed back to the model for one repair round); pure se ops auto-apply as one revision, store-aware se ops and every structure op become a proposal for the human Apply. Transcript = one `conv` per design, one block per answered turn tagged applied / proposal / rejected / no-op, `turn` handle `design-chat-<slug>~N`. → `src/precis_web/design_turn.py` · `src/precis_web/design_chat.py`
