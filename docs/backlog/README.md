...
- [`autocatpath-seed-wall-overruns`](./autocatpath-seed-wall-overruns.md) **idea** — 26 of 108 failures cluster at the runner's ~3 h deadline; the 5400 s compute
...
- [`budget-guardrails`](./budget-guardrails.md) **idea** — Piece B (global breaker) + real-cost capture are SHIPPED; the design doc's
...
- [`context-quality-eval`](./context-quality-eval.md) **idea** — Round-2 agent-facing render bugs, precisely root-caused, unfixed:
...
- [`diagram-editing-and-chunk-binding`](./diagram-editing-and-chunk-binding.md) **idea** — QuickJS engine lacks browser globals (offsetWidth, structuredClone, …).
...
- [`draft-inline-editor`](./draft-inline-editor.md) **idea** — virtual-scroller JS has no gate coverage and several browser-only bugs
- [`dreaming`](./dreaming.md) **idea** — / library-gap / open-question / consolidation / analogy), not just the
...
- [`email-kind`](./email-kind.md) **idea** — Slices 1–4 shipped (slice 4 inject_scan dark behind
...
- [`finding-chase`](./finding-chase.md) **idea** — The forward-bridge pilot (`src/precis/workers/chase.py::_taproot_bridge`) is
...
- [`fixer-salvage-failed-builds`](./fixer-salvage-failed-builds.md) **idea** — Pair push-on-NEEDS_YOU with branch GC so half-built branches don't
...
- [`graph-locality-framing`](./graph-locality-framing.md) **idea** — docs/design/graph-based.md proposes conditioning an agent's admissible
...
- [`health-watchdog`](./health-watchdog.md) **idea** — unalerted because every alerting path is DB-backed — an external SELECT 1
...
- [`local-first-capacity-valve`](./local-first-capacity-valve.md) **idea** — Item 3 (saturation→cloud spillover) is the open half; local serving shipped
...
- [`markup-first-ingest`](./markup-first-ingest.md) **idea** — Decide the PDF-race before flipping PRECIS_FETCH_MARKUP (still default-off):
...
- [`mcp-cold-start-token-budget`](./mcp-cold-start-token-budget.md) **idea** — OQ-11 (verification only; the design ships either way): does MCP 2025-06-18
...
- [`open-namespace-teardown`](./open-namespace-teardown.md) **idea** — Design awaiting Reto's review (open questions: `level:` axis-vs-column, the
...
- [`patent-authoring-loop`](./patent-authoring-loop.md) **idea** — ingest prior art (needs PRECIS_PATENT_RAW_ROOT + EPO OPS on the executor)
...
- [`pcb-0042-implementation`](./pcb-0042-implementation.md) **idea** — (real footprint conversion unwired — `src/precis/pcb/footprint.py` raises
...
- [`taproot-hub-refine`](./taproot-hub-refine.md) **idea** — Runbook: `docs/runbooks/taproot-chase-enablement.md`. The flip MUST be a
...
- [`topic-dossiers`](./topic-dossiers.md) **idea** — ensure_dossier + topic: tag + mark_weave_quest); the ADR-0060
