...
(`finding-trust-surfaces` (git-only), hub-unsupported deferral) a
...
- blocker: the central "flag it red" claim (Motivation's "surfaces as the
  ...
  (`finding-trust-surfaces` (git-only), "blocker 3 resolved", line
...
- `blocked-by: citation-bib-parse` resolves to a real sibling file
  (`citation-bib-parse` (git-only)) — not dangling. `BIBMARK:<version>`
