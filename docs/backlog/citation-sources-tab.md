...
writes a `list_bib_entries`-shaped accessor). `citation-bib-parse`'s
added `id` serial PK (noted in the task, spec shipped — git-only) doesn't
...
specs in `docs/backlog/`. `tests/precis_web/test_papers_refs.py`
