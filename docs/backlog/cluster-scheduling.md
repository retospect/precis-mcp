...
> items (now individual `docs/backlog/` files). Those remain the mechanical sub-specs (reconciliation at the
...
- **§L — The collapsed-worker cutover.** Apply the `20b` playbook: one
  ...
  **OS-agnostic** (systemd on the Linux nodes, not just launchd — `gr180078`);
  ...
  vault (`gr171431`).
  **Cycles a+b SHIPPED/EXECUTED** (2026-08-04; `df041901`, `901a7c26` —
  full story in git history): all hosts collapsed onto `20b`, split units
  retired, `site.yml`/`redeploy-precis.yml` import `20b`. Open residuals:
  the gr187627/gr191264 serial-rotation class (`chase` starving same-band
  reviewers under `--profile all`; gr192752's fix = `structural`/
  `deep_review` onto scheduler-lease cadences, §A pattern, not a band
  reorder); the gateway's `precis_agent_container_enabled` stays false
  pending the `dream_agent`-under-`PRECIS_AGENT_CONTAINER` smoke test.
...
- **§B-2 — Priority-claim + reserve mode (kill demoted). SHIPPED**
  (`de934b16` direction pin · `4b2824f5` reserve + kill): human-first
  `ORDER BY COALESCE(r.prio,5) ASC` claim (test-pinned), TTL'd
  `service='reserve'` gate on the heavy claim, `precis jobs kill` backstop
  with GPU reset. Present-tense detail: the executor docstrings.
...
- **§D — Liveness net. SHIPPED through Phase 2** (`a1d1573f`, `3424f110`:
  Layer-1/2 checks, cadence staleness, remediation router with
  condition-fingerprinted auto-closing gripes). Remaining under the §D
  umbrella: Phase 3 (brief lane, surface canaries, alert-triage
  disposition) and the P6 autonomy rungs. Full spec: `health-watchdog.md`.
- **§K — Factory console v2. SHIPPED** (`474b88d0`) — only the
  last-ok/fail click-through drill-down remains.
...
- `factory-console-and-scheduling.md` (docs/backlog/) → §K + the
