...
`struct_runs.params`, `llm_call_log.features` (`refs.authors` is done:
promoted to the `paper_authors` table in migration 0168, the jsonb is now
a projection regenerated by `store.set_paper_authors`).
