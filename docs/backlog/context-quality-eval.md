...
| ReMarkable reading send | `src/precis/workers/job_types/remarkable_reading_send.py::_dispatch` | `/env?agent=remarkable_reading_send` | Not an LLM context — deterministic per-source reading-edition typeset (`precis.export.reading_edition`) + compile + push to a paired reMarkable device. |
