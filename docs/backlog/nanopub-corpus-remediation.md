...
| 3 — title/body cohort repair | **not started**, and *smaller* than both earlier estimates: **45**, not 297 — the 297 was measured on `TAPROOT:claim` alone, which includes ~280 chase-tree findings that never mint. Strict (`+ STATUS:canonical`, n=1,249): 45 diverged, 42 title-longer, 3 body-longer, all August, **zero June**. So it is one cohort in one direction (chunk ← title) and the opposite-repair hazard does not apply to hubs. **0** are missing the `ord=0` chunk. Detail + the 3 to eyeball: `hub-title-body-chunk-divergence.md`. |
