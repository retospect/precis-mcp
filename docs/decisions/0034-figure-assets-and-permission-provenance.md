...
* Extended by: [0057 — figure medium axis + the source resolver](0057-figure-medium-axis-and-source-resolver.md) (splits `origin` into two axes — rights × medium — so a figure can be a blob / canvas / graph; `origin` here keeps its clearance meaning)
