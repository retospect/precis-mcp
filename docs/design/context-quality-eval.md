...
| context | producer | how to poke | what it ought to do |
|---|---|---|---|
| `view='dreamable'` — salience seed + ANN ring | `src/precis/runtime/angle.py::AngleMixin._dispatch_dreamable` | `search(kind='memory', view='dreamable')` | Surface the highest-salience unread neighborhood for a dream/reflection pass — see `docs/design/dreaming.md`. |
| `view='stubs'` — paper backlog | `src/precis/runtime/search.py::SearchMixin._dispatch_stubs` | `search(kind='paper', view='stubs')` | List papers with an external id but no PDF yet — the acquire backlog (`docs/design/stubs-mcp-and-skill.md`). |
...
soft-fail-to-gripe) follows `docs/design/docs-and-skills-redesign.md`
§"Quality gates".
