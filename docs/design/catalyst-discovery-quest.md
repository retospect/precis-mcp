...
- ~~The quest frontier ranks only `{energy, max_force, max_disp, n_steps}`~~
  **RESOLVED (Slice 1, built):** `_candidate_from_structure` now gathers
  ...
  and emits scalar `barrier`/`span`/`pathway_ref` onto its meta). What remains is
  the precis `dispatch_catpath` (mint that job under a candidate) + the quest.
...
| 3 | **The quest, first light** | precis-mcp (config) + tick + catpath bridge | **Harvest barrier-lift BUILT** (`TestCatpathHarvest`). **catpath emit-side BUILT + VERIFIED** — `catpath_explore` accepts `slab_extxyz` + `structure_ref` and emits scalar `barrier`/`span`/`pathway_ref` onto the job meta (the harvest contract); end-to-end checked in the dev container (injected Pd slab → EMT → `barrier` returned). **Remaining (precis-side):** `dispatch_catpath` (mint the job under a candidate: export its extxyz, create the pathway ref, parent the job on the candidate); mint the NO→NH₃/Pd quest (R decided, §8); the agentic **tick tools-loop** (§7.7); `rubric_objectives=[{barrier,min},{formation_e,min}]` + a graduation ceiling. Auto-loop stays dark. |
