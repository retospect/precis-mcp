...
| `PRECIS_DAILY_COST_CEILING` | Daily cost ceiling over *all* recorded LLM spend — gates the dispatcher at mint AND the scheduler's `spends=True` cadences at fire | `20.0` | The binding ceiling in practice: unlike the `PRECIS_BUDGET_DAILY_USD` breaker (real money only, `expensive` band only), this one counts notional `claude_agent`/`claude_p` OAuth dollars — the bulk of recorded spend. Set it with real headroom over a busy day (prod: `precis_daily_cost_ceiling` in `deploy/group_vars/all.yml`, 100.0 since 2026-08-17). Cadence work and zero-LLM compute mints are exempt (`workers/dispatch.py`). |
| `PRECIS_SUSPENDED_JOB_TYPES` | Operator hold: comma-separated job_types the claim path never claims (queued rows wait; in-flight rows finish) and the dispatcher/quest tick won't mint | *(empty)* | Deploy var `precis_suspended_job_types` (`deploy/group_vars/all.yml`); survives redeploys, resume = set `""` + redeploy. Used 2026-08-17 to suspend `autocatpath_seed,autocatpath_aggregate`. |
