...
the dark-switch audit into one of three buckets.
...
| Flag | Class | Why |
|---|---|---|
| `PRECIS_LAYER2_FIXER` | orphaned/superseded (harmless, kept dark by choice) | The Layer-2 chktex LLM-fixer on the `kind='tex'` put path (`utils/tex_llm_fix.py`, one caller in `handlers/plaintext.py`). Drafts are now the authoring source of truth, so this hook is likely superseded — but it's self-contained and harmless, so the backlog (`docs/backlog/tex-layer2-fixer-fate.md`) says leave it dark and decide keep-vs-delete deliberately later (removing it also drops the Layer-2 fix-*hint* on tex puts, a real behavior change, not a mechanical rip). |
| `PRECIS_BACKLOG_GROOM_ENABLED` | intentional-staged | `workers/backlog_groom.py` (gripe→`fix_gripe`-todo groomer) is fully wired (`cli/worker.py`, `workers/registry.py`) with an explicit documented activation step (`docs/backlog/backlog-groomer-items-half.md`): flip it on a system worker once ready to drain open gripes, watch mint count + fixer throughput before widening. |
| `PRECIS_FRICTION_REFLECT` | intentional-staged | End-of-run tool-friction footer (`utils/friction_reflect.py`, wired into `utils/claude_agent.py`). the backlog names the explicit prerequisite (a downstream grouping/dedup lane for `friction` gripes) that gates flipping it on — staged on purpose, not forgotten. |
| `ROLE3:own` (chunk-tag classify pass) | intentional-staged | Not itself an env var — a tag value the classify pass (`workers/classify.py`) writes when `PRECIS_CLASSIFY_ENABLED` (also default-off) runs. the backlog (`docs/backlog/dark-features-activation.md`) documents the activation path (flip `PRECIS_CLASSIFY_ENABLED=1` to drain the corpus) and the gold-set-classify memory notes full-corpus commit awaits a passing model + Reto's go-ahead — a deliberate gate, not an orphan. |
| `PRECIS_AGENT_CONTAINER` | intentional-staged | The §13 container-agent executor. the backlog records: image built, distributed, and smoke-proven on melchior; "Flip is the window action" — deliberately staged behind the flag pending the Phase-2 deploy window. |
| `PRECIS_SCHEDULER_ENABLED` | **resolved** — flag retired, no longer exists | Was intentional-staged at audit time; the §A cutover (2026-08) flipped the Phase-2 window: the `scheduler` `ServiceSpec` dropped `enable_env` entirely and is default-on for both worker profiles (`workers/registry.py`), and `retire-thin-timers.yml` boots out the standalone launchd timers it replaces. Row kept for the audit's historical record. |
...
| `PRECIS_LLM_BACKEND` | intentional, not a "dark" flag | A live mode selector (default `anthropic`, can be set to `openai` to reroute the LLM router at an OSS/hosted endpoint) — actively used across many call sites (`workers/dream_agent.py`, `workers/review.py`, `workers/job_types/*`, `precis_web/ask.py`, …) as the deliberate "switch the whole pass to a different backend without a redeploy" seam (the LLM routing seam). Not a candidate for removal; it's infrastructure, not a forgotten hook. |
...
`docs/backlog/budget-guardrails.md` for the residual pieces (the
