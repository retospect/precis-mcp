...
| Marketing/demo surface (not dev docs, not the in-app manual) | `guide/` — built by `guide/build.py` into `guide/index.html` (published via `.github/workflows/pages.yml`); assets (`guide/assets/`) are generated by `scripts/guide-capture` / `scripts/guide-annotate` / `scripts/guide-narrate`, never hand-edited |
