...
add a **closed discriminator tag** `TAPROOT:claim` vs `TAPROOT:review`.
`canon.block()` already queries `TAPROOT:claim` hubs — so this slice
immediately makes live canonicalization real.

## Slice 2a — TAPROOT classifier
...
- **`src/precis/data/axes/taproot.yaml`** — `id: taproot`, `version: 1`,
...
- **`src/precis/taproot/hub.py`** (new — the single write door):
  - `mint_hub(store, claim, *, set_by="agent") -> int` — create a `finding`
    (claim sentence → title, `claim.scope` → `meta.scope`) + tag `TAPROOT:claim`.
  - `attach_evidence(store, *, hub_ref_id, paper_ref_id, role, meta)` —
    validate `role` via `validate_relation`, guard `hub_ref_id` is a
    `TAPROOT:claim` finding, then `add_link(src=paper, dst=hub, relation=role)`.
...
- **Tests** (`tests/test_taproot_hub.py`, offline, test DB): mint creates a
  `TAPROOT:claim` finding; attach writes each role, rejects an unknown role and
...
- `taproot.yaml` classifies synthetic findings claim/review in tests; `TAPROOT`
...
- The migration seeds the 4 relation slugs (`store.valid_relations()`
  contains them); `hub.py` mints a `TAPROOT:claim` hub and attaches all three
