...
> the catpath pipeline (`docs/design/autocatpath-integration.md`). Siblings:
> `bundle-pathway-in-tree-plugin.md` (relocates the glue this touches — see
> Cutover interaction) and `gpu-priority.md` (re-chunks the job this hooks — see
...
- **Containerizing compute, or moving the science engine** — orthogonal
  (`bundle-pathway-in-tree-plugin.md` owns code-home).
...
- **Docs:** `docs/design/autocatpath-integration.md` (capture seam), ADR 0043
...
- **NAS retention / GC.** `/botshome` is occasionally backed up; frames are
  ...
  open.
- **Cutover interaction with `bundle-pathway-in-tree-plugin.md`.** The science
  edits (`neb.py`/`relax.py`) stay in catpath regardless. The glue edits
  (`persist/ingest/job`) target `catpath/precis/` **pre-bundle** or
  `src/precis_pathway/` **post-bundle** — same code, different home. Not a hard
  `blocked-by`; if bundling ships first, update the glue paths. Sequencing note,
  not a block.
