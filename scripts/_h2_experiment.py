...
Three configurations:
  ...
  B — long descriptive H2 + body (proposed)
...
# (doc, short_h2, long_h2, body) per section
SECTIONS = [
    # search-help
...
def build_corpus(mode: str) -> list[str]:
    ...
    docs = []
    for _, short_h2, long_h2, body in SECTIONS:
        if mode == "A":
            docs.append(f"{short_h2}\n\n{body}")
        elif mode == "B":
            docs.append(f"{long_h2}\n\n{body}")
        elif mode == "C":
            docs.append(long_h2)
        else:
...
def evaluate(model: SentenceTransformer, mode: str) -> dict[str, float]:
    ...
    for i, (q, expected) in enumerate(QUERIES):
        ...
        if rank > 1:
            top = ranked[0]
            misses.append(f"  q={q!r}\n    expected #{expected} ({SECTIONS[expected][1]}), got rank {rank}; top was #{top} ({SECTIONS[top][1]})")
    return {
        "P@1": p1 / n,
        "P@3": p3 / n,
        "MRR": mrr / n,
...
def main() -> None:
    ...
    for mode in ("A", "B", "C"):
        ...
        label = {
            ...
            "B": "B: long H2 + body (proposed)",
            "C": "C: long H2 only (no body)",
        }[mode]
        print(f"{label}")
