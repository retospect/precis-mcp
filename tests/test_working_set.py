...
def _eye(ws: WorkingSet, handle: str) -> Eye:
    """``ws.get`` narrowed for tests — asserts the eye is actually there."""
    eye = ws.get(handle)
...
def test_hop1_decay_peels_the_ring_first() -> None:
    ...
    assert _eye(ws, "pc1").extent is Extent.HOP1
    ws.age(4)
    demoted, dropped = ws.crunch()
    assert demoted == ["pc1"] and dropped == []
    assert _eye(ws, "pc1").extent is Extent.FIDELITY  # ring peeled, neighborhood kept
    ws.age(4)
    ws.crunch()
    assert _eye(ws, "pc1").extent is Extent.TOC  # then down to kwd
    ws.age(4)
    _, dropped = ws.crunch()
...
def test_focus_places_and_replaces_one_eye_per_handle() -> None:
    ...
    assert _eye(ws, "pc12").extent is Extent.SUMMARY
    # re-focus replaces (the refresh/adopt action, §6)
    ws.focus("pc12", "full")
    assert len(ws.eyes) == 1
    assert _eye(ws, "pc12").extent is Extent.FULL
...
def test_pin_never_decays() -> None:
    ws = WorkingSet()
    ws.pin("pe3", "fidelity")
    assert _eye(ws, "pe3").persistence is Persistence.PINNED
...
def test_snapshot_round_trips_through_meta() -> None:
    ...
    assert _eye(restored, "pc12").extent is Extent.FULL
    assert _eye(restored, "pc88").persistence is Persistence.TRANSIENT
    assert _eye(restored, "pe1").persistence is Persistence.PINNED
...
def test_focus_derives_ttl_from_persistence() -> None:
    ...
    assert _eye(ws, "pc1").ttl == 4
    assert _eye(ws, "pc2").ttl == 1
...
def test_age_ripens_but_does_not_drop_and_spares_pinned() -> None:
    ...
    ws.age(2)
    assert _eye(ws, "pc1").ttl == 2
    assert _eye(ws, "pe1").ttl is None  # pinned untouched
    assert ws.expiring() == []  # nothing ripe yet
    # floor at 0, never negative
    ws.age(10)
    assert _eye(ws, "pc1").ttl == 0
...
def test_crunch_demotes_a_neglected_normal_eye_then_drops_it() -> None:
    ...
    demoted, dropped = ws.crunch()
    assert demoted == ["pc1"] and dropped == []
    assert _eye(ws, "pc1").extent is Extent.TOC  # full → toc (first warn)
    assert _eye(ws, "pc1").ttl == 4  # refreshed at the new rung
...
def test_crunch_never_touches_pinned() -> None:
    ...
    assert ws.expiring() == []
    assert ws.crunch() == ([], [])
    assert _eye(ws, "pe1").persistence is Persistence.PINNED
