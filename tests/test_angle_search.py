...
def _embed_card(store: Store, ref_id: int, text: str) -> int:
    cid = store.chunks.upsert_card_combined(ref_id, text)
    with store.pool.connection() as conn:
        conn.execute(
            "INSERT INTO chunk_embeddings (chunk_id, embedder, vector, status, attempts) "
            "VALUES (%s, %s, %s, 'ok', 1)",
...
def test_get_chunk_vector_round_trips(store: Store) -> None:
    ...
    got = store.chunks.get_chunk_vector(cid)
...
def test_get_chunk_vector_none_when_unembedded(store: Store) -> None:
    ...
    assert row is not None
    assert store.chunks.get_chunk_vector(int(row[0])) is None
...
def test_seed_chunk_for_ref_prefers_card(store: Store) -> None:
    ...
    assert store.chunks.seed_chunk_for_ref(ref.id) == card
...
def test_angle_one_snaps_to_the_seed_item(store: Store) -> None:
    ...
    hits = store.chunks.angle_neighbours(vec, angle=1.0, n=1, rng=random.Random(0))
    assert len(hits) == 1
...
def test_results_are_distinct(store: Store) -> None:
    ...
    hits = store.chunks.angle_neighbours(vecs[0], angle=0.3, n=5, rng=random.Random(1))
...
def test_empty_seed_returns_empty(store: Store) -> None:
    assert store.chunks.angle_neighbours([], angle=1.0, n=4) == []
...
def test_deterministic_under_seeded_rng(store: Store) -> None:
    ...
    a = store.chunks.angle_neighbours(vecs[0], angle=0.4, n=4, rng=random.Random(7))
    b = store.chunks.angle_neighbours(vecs[0], angle=0.4, n=4, rng=random.Random(7))
