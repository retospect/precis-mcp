...
def test_bump_advances_last_seen_and_accesses(store: Store) -> None:
    ...
    n = store.chunks.bump_salience([cid])
    assert n == 1
...
def test_bump_empty_is_noop(store: Store) -> None:
    assert store.chunks.bump_salience([]) == 0
...
def test_dream_actor_reads_do_not_bump(store: Store) -> None:
    ...
    with as_dream_actor():
        assert store.chunks.bump_salience([cid]) == 0
...
def test_select_dream_seed_picks_argmax_and_rotates(store: Store) -> None:
    ...
    assert store.chunks.select_dream_seed() == ca
    # surfacing ca stamps last_dreamt → cb (untouched, score 0) now tops
    store.chunks.touch_last_dreamt([ca])
...
def test_select_dream_seed_excludes_non_target_kinds(store: Store) -> None:
    ...
    store.chunks.bump_salience([ct])  # hot, but a todo is never a dream target
    assert store.chunks.select_dream_seed() != ct
...
def test_select_dream_seed_respects_kind_filter(
    ...
    cm = store.chunks.upsert_card_combined(mem.id, "m")
    _embed(store, cm, "m")  # dream seed is embedded_only (gr48249)
    store.chunks.bump_salience([cp, cm])
    seed = store.chunks.select_dream_seed(kinds=kinds)
...
def test_select_salient_watch_argmax_and_limit(store: Store) -> None:
    ...
    ca = _mk_chunk(store, pa.id, 0, "a")
    cb = _mk_chunk(store, pb.id, 0, "b")
    store.chunks.bump_salience([ca])  # ca hottest
    assert store.chunks.select_salient("watch", kinds=("paper",), limit=1) == [ca]
    # limit returns top-N most-due, ca first
    top2 = store.chunks.select_salient("watch", kinds=("paper",), limit=2)
...
def test_dream_and_watch_rotate_independently(store: Store) -> None:
    ...
    ca = _mk_chunk(store, pa.id, 0, "a")
    cb = _mk_chunk(store, pb.id, 0, "b")
    store.chunks.bump_salience([ca])  # ca hottest for BOTH actors
    assert store.chunks.select_salient("dream", kinds=("paper",))[0] == ca
    assert store.chunks.select_salient("watch", kinds=("paper",))[0] == ca
    # Dreamer rotates ca out → cb tops for dream...
    store.chunks.touch_attended("dream", [ca])
    assert store.chunks.select_salient("dream", kinds=("paper",))[0] == cb
    # ...but the watcher still sees ca as most-due (independent stamp).
    assert store.chunks.select_salient("watch", kinds=("paper",))[0] == ca
    # Watcher rotates ca out on its own clock → cb tops for watch too.
    store.chunks.touch_attended("watch", [ca])
    assert store.chunks.select_salient("watch", kinds=("paper",))[0] == cb


def test_touch_attended_unknown_actor_raises(store: Store) -> None:
    with pytest.raises(KeyError):
        store.chunks.touch_attended("bogus", [1])
    with pytest.raises(KeyError):
        store.chunks.select_salient("bogus", kinds=("paper",))
...
def test_touch_last_dreamt_still_works_via_wrapper(store: Store) -> None:
    ...
    store.chunks.bump_salience([ca])
    store.chunks.touch_last_dreamt([ca])
    assert store.chunks.select_dream_seed(kinds=("paper",)) == cb  # dream rotated to cb
    assert (
        store.chunks.select_salient("watch", kinds=("paper",))[0] == ca
    )  # watch untouched
...
def test_dream_seed_overweights_draft_within_boost(store: Store, monkeypatch) -> None:
    ...
    assert store.chunks.select_dream_seed(kinds=_DREAM_KINDS) == cd
...
def test_dream_seed_paper_wins_when_far_overdue(store: Store, monkeypatch) -> None:
    ...
    assert store.chunks.select_dream_seed(kinds=_DREAM_KINDS) == cp
