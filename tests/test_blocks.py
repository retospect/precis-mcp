...
class TestInsertBlocks:
    def test_basic_insert(self, store: Store) -> None:
        ...
        result = store.blocks.insert_blocks(ref_id, blocks)
        assert len(result) == 2
    ...
    def test_empty_list_no_op(self, store: Store) -> None:
        ref_id = _paper_ref(store)
        result = store.blocks.insert_blocks(ref_id, [])
        assert result == []
        assert store.blocks.count_blocks(ref_id) == 0
    ...
    def test_replace_drops_old_blocks(self, store: Store) -> None:
        ...
        assert store.blocks.count_blocks(ref_id) == 2

        store.blocks.insert_blocks(
            ref_id, [BlockInsert(pos=0, text="new")], replace=True
        )
        assert store.blocks.count_blocks(ref_id) == 1
        block = store.blocks.get_block(ref_id, pos=0)
    ...
    def test_in_existing_transaction(self, store: Store) -> None:
        ...
        with store.tx() as conn:
            store.blocks.insert_blocks(
                ref_id, [BlockInsert(pos=0, text="atomic")], conn=conn
            )
        assert store.blocks.count_blocks(ref_id) == 1

    def test_pos_uniqueness_enforced(self, store: Store) -> None:
        ref_id = _paper_ref(store)
        store.blocks.insert_blocks(ref_id, [BlockInsert(pos=0, text="a")])
        with pytest.raises(psycopg.errors.UniqueViolation):
            store.blocks.insert_blocks(ref_id, [BlockInsert(pos=0, text="b")])
...
class TestGetBlock:
    def test_by_pos(self, store: Store) -> None:
        ref_id = _paper_ref(store)
        store.blocks.insert_blocks(ref_id, [BlockInsert(pos=5, text="five")])
        block = store.blocks.get_block(ref_id, pos=5)
        assert block is not None
    ...
    def test_by_slug(self, store: Store) -> None:
        ref_id = _paper_ref(store)
        store.blocks.insert_blocks(
            ref_id, [BlockInsert(pos=0, text="hi", slug="HELLO")]
        )
        block = store.blocks.get_block(ref_id, slug="HELLO")
        assert block is not None
        assert block.slug == "HELLO"

    def test_returns_none_when_missing(self, store: Store) -> None:
        ref_id = _paper_ref(store)
        assert store.blocks.get_block(ref_id, pos=99) is None

    def test_requires_exactly_one_locator(self, store: Store) -> None:
        ref_id = _paper_ref(store)
        with pytest.raises(BadInput):
            store.blocks.get_block(ref_id)
        with pytest.raises(BadInput):
            store.blocks.get_block(ref_id, pos=0, slug="x")
    ...
    def test_embedding_excluded_by_default(self, store: Store) -> None:
        ...
        block_with = store.blocks.get_block(ref_id, pos=0, with_embedding=True)
...
class TestListBlocks:
    ...
    def test_pos_range_inclusive(self, store: Store) -> None:
        ...
        blocks = store.blocks.list_blocks_for_ref(ref_id, pos_range=(3, 5))
    ...
    def test_count_blocks(self, store: Store) -> None:
        ...
        assert store.blocks.count_blocks(ref_id) == 7


class TestUpdateBlock:
    def test_update_density(self, store: Store) -> None:
        ref_id = _paper_ref(store)
        result = store.blocks.insert_blocks(
            ref_id, [BlockInsert(pos=0, text="x", density="medium")]
        )
        store.blocks.update_block_density(result[0].id, "dense")
        block = store.blocks.get_block(ref_id, pos=0)
        assert block is not None
    ...
    def test_update_embedding(self, store: Store) -> None:
        ref_id = _paper_ref(store)
        result = store.blocks.insert_blocks(ref_id, [BlockInsert(pos=0, text="x")])
        # Initially no embedding.
        block = store.blocks.get_block(ref_id, pos=0, with_embedding=True)
        assert block is not None
        assert block.embedding is None

        embedder = MockEmbedder(dim=1024)
        store.blocks.update_block_embedding(result[0].id, embedder.embed_one("x"))
        block = store.blocks.get_block(ref_id, pos=0, with_embedding=True)
        assert block is not None
...
class TestBlocksMissingEmbeddings:
    def test_filters_by_null(self, store: Store) -> None:
        ...
        missing = store.blocks.blocks_missing_embeddings(kind="paper")
    ...
    def test_skips_deleted_refs(self, store: Store) -> None:
        ...
        store.blocks.insert_blocks(ref_id, [BlockInsert(pos=0, text="x")])
        dead = store.insert_ref(kind="paper", slug="dead", title="Dead")
        store.blocks.insert_blocks(dead.id, [BlockInsert(pos=0, text="y")])
    ...
    def test_kind_filter(self, store: Store) -> None:
        ...
        store.blocks.insert_blocks(paper.id, [BlockInsert(pos=0, text="paper text")])
        store.blocks.insert_blocks(memory.id, [BlockInsert(pos=0, text="mem text")])

        paper_only = store.blocks.blocks_missing_embeddings(kind="paper")
...
class TestCountRefsMatchingLexical:
    ...
    def test_counts_distinct_matching_refs_across_kinds(self, store: Store) -> None:
        ...
        total = store.blocks.count_blocks_lexical(
    ...
    def test_no_match_returns_zero(self, store: Store) -> None:
        ...
        store.blocks.insert_blocks(
            ref_id, [BlockInsert(pos=0, text="unrelated content")]
        )
        total = store.blocks.count_blocks_lexical(
            kinds=["paper"], q="xenocryst", distinct_refs=True
        )
    ...
    def test_empty_kinds_or_blank_q_returns_zero(self, store: Store) -> None:
        ...
        store.blocks.insert_blocks(ref_id, [BlockInsert(pos=0, text="xenocryst text")])
        assert (
        ...
        assert (
            store.blocks.count_blocks_lexical(kinds=["paper"], q="", distinct_refs=True)
            == 0
        )
...
class TestChunkSummariesBulk:
    def test_chunk_summaries_bulk_maps_ref_ord_pairs(self, store: Store) -> None:
        ...
        blocks_a = store.blocks.insert_blocks(
            ref_a,
            [BlockInsert(pos=0, text="a0"), BlockInsert(pos=1, text="a1")],
        )
        blocks_b = store.blocks.insert_blocks(ref_b, [BlockInsert(pos=1, text="b1")])
        with store.pool.connection() as conn:
            conn.execute(
        ...
        out = store.blocks.chunk_summaries_bulk([(ref_a, 0), (ref_b, 1), (ref_a, 99)])
        assert out == {
    ...
    def test_chunk_summaries_bulk_empty_pairs(self, store: Store) -> None:
        assert store.blocks.chunk_summaries_bulk([]) == {}
