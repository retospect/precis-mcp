...
def test_ingest_sim_creates_markdown_and_plaintext_refs(
    ...
    assert store.chunks.count_chunks(md_ref.id) > 0
    ...
    assert store.chunks.count_chunks(txt_ref.id) > 0
...
def test_ingest_sim_second_run_over_unchanged_files_is_a_no_op(
    ...
    md_blocks_before = store.chunks.count_chunks(md_ref.id)
    txt_blocks_before = store.chunks.count_chunks(txt_ref.id)
    ...
    assert store.chunks.count_chunks(md_ref_after.id) == md_blocks_before
    assert store.chunks.count_chunks(txt_ref_after.id) == txt_blocks_before
