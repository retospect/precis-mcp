"""Universal-handle resolution + surface dispatch — DB-backed.
...
def test_resolve_handle_slug_kind(store: Store) -> None:
    ref = store.insert_ref(kind="oracle", slug="uh36-oracle", title="o")
    h = handle_registry.format_handle("oracle", ref.id)  # 'or<ref_id>'
    assert h[:2] == handle_registry.KIND_CODES["oracle"]
    resolved = store.resolve_handle(h)
    assert resolved is not None
    assert resolved.public_id == "uh36-oracle"  # slug kind → slug
...
# --- chunk handles (universal handles — computed from chunk_id) -------------------
...
def test_chunk_handle_resolves_to_chunk_and_owning_ref(store: Store) -> None:
    ref = store.insert_ref(kind="paper", slug="uh36-chunk-paper", title="p")
    ...
    assert resolved.public_id == "uh36-chunk-paper"  # owning ref's slug
...
def test_chunk_handle_kind_mismatch_is_rejected(store: Store) -> None:
    ...
    ref = store.insert_ref(kind="paper", slug="uh36-mismatch", title="p")
    chunk_id = _insert_chunk(store, ref.id)
...
def test_render_prefers_computed_uhandle_over_legacy() -> None:
    ...
    # handle and NOT the legacy slug~pos (cutover).
...
# --- input acceptance (cutover): handles round-trip ------------
...
def test_parse_link_target_accepts_chunk_handle(store: Store) -> None:
    ...
    ref = store.insert_ref(kind="paper", slug="uh36-link-chunk", title="p")
...
def test_paper_exclude_accepts_handle(store: Store) -> None:
    ...
    assert _normalise_exclude_slug(h, store=store) == "uh36-excl"
    # A record handle too.
    rh = handle_registry.format_handle("paper", ref.id)
    assert _normalise_exclude_slug(rh, store=store) == "uh36-excl"


# --- relative navigation — flat ord kinds --------------------
...
def test_resolve_relative_sibling_steps(store: Store) -> None:
    ...
    assert store.chunks.resolve_relative(f"{h1}+1") == ("paper", "uh36-rel~2")
    assert store.chunks.resolve_relative(f"{h1}+2") == ("paper", "uh36-rel~3")
    assert store.chunks.resolve_relative(f"{h1}-1") == ("paper", "uh36-rel~0")
    assert store.chunks.resolve_relative(f"{h1}++") == ("paper", "uh36-rel~2")
...
def test_resolve_relative_zero_step_is_the_chunk_itself(store: Store) -> None:
    ...
    h1 = handle_registry.format_handle("paper", cids[1], chunk=True)  # ord 1
    assert store.chunks.resolve_relative(f"{h1}-0") == ("paper", "uh36-rel-zero~1")
    assert store.chunks.resolve_relative(f"{h1}+0") == ("paper", "uh36-rel-zero~1")


def test_resolve_relative_out_of_range_is_none(store: Store) -> None:
    ref = store.insert_ref(kind="paper", slug="uh36-rel-edge", title="p")
    cids = [_insert_chunk(store, ref.id, ord_=i) for i in range(3)]
    ...
    assert store.chunks.resolve_relative(f"{last}+1") is None  # past the end
    assert store.chunks.resolve_relative(f"{first}-1") is None  # before the start
...
def test_resolve_relative_span_clamps(store: Store) -> None:
    ...
    assert store.chunks.resolve_relative(f"{h2}-1..1") == ("paper", "uh36-span~1..3")
    # clamps to the document bounds [0, 4]
    assert store.chunks.resolve_relative(f"{h2}-9..9") == ("paper", "uh36-span~0..4")
...
def test_resolve_relative_ancestor_on_flat_kind_is_none(store: Store) -> None:
    ...
    assert store.chunks.resolve_relative(f"{h}^") is None  # papers have no hierarchy
...
def test_resolve_relative_non_relative_is_none(store: Store) -> None:
    ...
    cid = _insert_chunk(store, ref.id, ord_=0)
    h = handle_registry.format_handle("paper", cid, chunk=True)
    assert store.chunks.resolve_relative(h) is None  # absolute handle, no operator
...
def test_surface_record_handle_carries_chunk_selector(
    ...
    ref = store.insert_ref(kind="paper", slug="uh36-pa-sel", title="p")
    for i in range(3):
    ...
    assert runtime_with_store.dispatch("get", {"id": f"{pa}~0..2"}) == (
        runtime_with_store.dispatch("get", {"id": "uh36-pa-sel~0..2"})
    )
...
def test_surface_get_relative_routes_to_sibling(
    ...
    ref = store.insert_ref(kind="paper", slug="uh36-rel-surface", title="p")
    cids = [_insert_chunk(store, ref.id, ord_=i) for i in range(3)]
    ...
    via_selector = runtime_with_store.dispatch("get", {"id": "uh36-rel-surface~1"})
...
def test_surface_get_chunk_handle_routes_to_selector(
    ...
    ref = store.insert_ref(kind="paper", slug="uh36-chunk-surface", title="p")
    ...
    via_handle = runtime_with_store.dispatch("get", {"id": h})
    via_selector = runtime_with_store.dispatch("get", {"id": "uh36-chunk-surface~0"})
...
def test_surface_get_zero_step_routes_to_the_chunk(
    ...
    ref = store.insert_ref(kind="paper", slug="uh36-zero-surface", title="p")
    chunk_id = _insert_chunk(store, ref.id, ord_=0)
    h = handle_registry.format_handle("paper", chunk_id, chunk=True)
    via_zero = runtime_with_store.dispatch("get", {"kind": "paper", "id": f"{h}-0"})
    via_handle = runtime_with_store.dispatch("get", {"kind": "paper", "id": h})
    via_selector = runtime_with_store.dispatch("get", {"id": "uh36-zero-surface~0"})
