...
def test_kind_is_available_without_any_flag(hub: Hub) -> None:
    """`se` carries no private enable flag — it is on wherever the plugin
    ...
    assert SeHandler.spec.requires_setting == ()
    assert SeHandler.spec.is_available() is True
    SeHandler(hub=hub)
