...
def _diagram_and_targets(store: Store) -> tuple[int, str, str]:
    ...
    dia, dtitle = store.drafts.create_draft(
    ...
    order = store.drafts.reading_order(src.id)
    return node, order[1].dc, order[2].dc  # two paragraph handles


def _bindings(store: Store, node: int) -> set[tuple[str, str]]:
    return {(b["element"], b["handle"]) for b in store.drafts.element_bindings(node)}
...
def test_bind_two_elements_same_target_share_one_row(store: Store) -> None:
    ...
    store.drafts.bind_element(node_chunk_id=node, element="deckhook", target=h1)
    store.drafts.bind_element(node_chunk_id=node, element="load-arrow", target=h1)
    store.drafts.bind_element(node_chunk_id=node, element="latch", target=h2)
...
def test_unbind_keeps_row_until_last_element(store: Store) -> None:
    ...
    assert (
        store.drafts.unbind_element(node_chunk_id=node, element="deckhook", target=h1)
        == 1
    )
    ...
    store.drafts.unbind_element(node_chunk_id=node, element="latch")
    assert _depicts_rows(store, node) == 1
...
def test_bind_ref_level_target_memory(store: Store) -> None:
    ...
    store.drafts.bind_element(node_chunk_id=node, element="load-arrow", target=mh)

    got = store.drafts.element_bindings(node)
    assert got == [
        {
            "element": "load-arrow",
            "relation": "depicts",
...
def test_set_element_bindings_reconciles(store: Store) -> None:
    node, h1, h2 = _diagram_and_targets(store)
    store.drafts.bind_element(node_chunk_id=node, element="a", target=h1)

    res = store.drafts.set_element_bindings(
        node_chunk_id=node,
    ...
    res2 = store.drafts.set_element_bindings(node_chunk_id=node, desired=[])
...
def test_bind_bad_input_raises(store: Store) -> None:
    ...
    with pytest.raises(BadInput):
        store.drafts.bind_element(node_chunk_id=node, element="e", target="dc999999999")
    with pytest.raises(BadInput):
        store.drafts.bind_element(node_chunk_id=node, element="  ", target=h1)
